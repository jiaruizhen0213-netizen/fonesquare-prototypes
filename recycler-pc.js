...
function renderDetail(){const t=current();if(!t){selected=null;render();return;}
 ...
 <div class="small-section"><h3>质检报告 <span class="sub">只读</span></h3>${reportLink('MAX 原始报告','max','MAX-881920 / v4')}${reportLink('二级质检报告','secondary','PMS-'+t.id.slice(-4)+' / v1')}${reportLink('拍机堂质检报告1','r1','PJT1-'+t.id.slice(-4)+' / v1')}</div>
