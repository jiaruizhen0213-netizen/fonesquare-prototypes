...
input('[data-row="0"] [data-field="startRemainingPercent"]','101');click('[data-action="save"]');assert.match(el('#error').textContent,/开始时间比例/);click('[data-action="restore"]');
assert.equal(el('[data-row="0"] [data-field="endRemainingPercent"]').value,'90');
input('[data-row="0"] [data-field="endRemainingPercent"]','100');click('[data-action="save"]');assert.match(el('#error').textContent,/大于结束/);click('[data-action="restore"]');
input('[data-row="0"] [data-field="endRemainingPercent"]','95');click('[data-action="save"]');assert.equal(el('#error').textContent,'');
click('[data-action="history"]');assert.match(el('.history-card').textContent,/结束时间比例/);click('[data-action="history"]');
input('[data-row="0"] [data-field="bidCount"]','1.5');click('[data-action="save"]');assert.match(el('#error').textContent,/正整数/);input('[data-row="0"] [data-field="bidCount"]','6');change('#enabled','true');click('[data-action="save"]');assert.match(el('.version').textContent,/v3/);assert.equal(el('#enabled').value,'true');assert.equal(el('[data-row="2"] [data-field="firstBidPercent"]').value,'95');
assert.equal(w.localStorage.getItem('fs-quote-config-demo-v1'),legacy);const persisted=JSON.parse(w.localStorage.getItem('fs-quote-config-v2'));assert.equal(persisted.current.rows[0].bidCount,6);assert.equal(persisted.current.rows[0].endRemainingPercent,95);
...
const reloaded=await JSDOM.fromURL('http://127.0.0.1:8796/quote-config.html',{resources:'usable',runScripts:'dangerously',beforeParse(rw){rw.localStorage.setItem('fs-quote-merchants-v1',merchantSaved);rw.localStorage.setItem('fs-quote-config-v2',JSON.stringify(persisted));}});await new Promise(r=>reloaded.window.addEventListener('load',r));assert.match(reloaded.window.document.querySelector('.merchant-table').textContent,/101.7116/);assert.equal(reloaded.window.document.querySelector('[data-row="0"] [data-field="endRemainingPercent"]').value,'95');reloaded.window.close();
