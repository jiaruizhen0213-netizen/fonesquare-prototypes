...
test('remaining time ranges validate endpoints, order and overlap',()=>{
 ...
 for(const v of [-1,101,NaN])assert.match(C.validate([row(100,v)]),/结束时间比例/);
...
test('single-value migration leaves ends unset and history untouched',()=>{
 ...
 assert.equal(JSON.stringify(old),before);assert.equal(next.schema,3);assert.equal(next.current.rows[0].startRemainingPercent,50);assert.equal(next.current.rows[0].endRemainingPercent,null);assert.deepEqual(next.history,old.history);assert.equal(C.validate(next.current.rows),'');
});
...
test('only final range may have an empty end; save and clone retain null',()=>{
 const rows=C.initialRows();assert.equal(rows.at(-1).endRemainingPercent,null);assert.equal(C.validate(rows),'');
 rows[0].endRemainingPercent=null;assert.match(C.validate(rows),/第1行.*结束时间比例/);
 const state=C.seed(),next=C.save(state,state.current,'now');assert.equal(next.current.rows.at(-1).endRemainingPercent,null);
 const old=C.seed();old.current.rows.at(-1).endRemainingPercent=0;const migrated=C.migrate(old);assert.equal(migrated.current.rows.at(-1).endRemainingPercent,null);assert.equal(old.current.rows.at(-1).endRemainingPercent,0);
});
