...
(async()=>{
 ...
 try {
  ...
  await screenshot('mobile-code');await action('terms');assert.ok(await q('#auth-policy').isVisible());await action('close');
  await action('password');await screenshot('phone-password');await action('switch');await screenshot('email-password');
  ...
  await action('password');await action('switch');await q('#auth-address').fill('sample@example.com');await action('reset');await q('#auth-consent').check();await q('#auth-password').fill('NewDemo123');await q('#auth-code').fill('123456');await q('#auth-submit').click();assert.ok((await q('#auth-error').innerText()).includes('先获取'));await action('send');await q('#auth-submit').click();assert.ok((await q('#auth-error').innerText()).includes('已重置'));assert.ok(await q('#quote-workspace').isHidden());
  // Registration requires code and password, uses only a simulated session.
  await action('register');await q('#auth-password').fill('DemoRegister123');await action('send');await q('#auth-code').fill('123456');await q('#auth-submit').click();assert.ok(await q('#quote-workspace').isVisible());await q('#auth-logout').click();
  await action('language');assert.ok((await q('#auth-heading, .auth-heading').innerText()).includes('Sign in'));await action('password');await action('switch');assert.equal(await q('#auth-address').getAttribute('type'),'email');await action('language');
