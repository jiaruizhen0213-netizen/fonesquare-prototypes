...
(async()=>{
 ...
 try {
  ...
  await action('language');assert.equal(await q('#auth-submit').innerText(),'Sign in / Register');await action('password');await action('switch');assert.equal(await q('#auth-address').getAttribute('type'),'email');await action('language');
