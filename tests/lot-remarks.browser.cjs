...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('#lotRemark').getAttribute('required'), '');
    assert.ok((await page.locator('#lotRemarkHelp').innerText()).includes('英文'));
    ...
    assert.equal(await page.locator('#lotRemark').getAttribute('aria-invalid'), 'true');
    const note = 'Minor frame scratches\n<img src=x onerror=alert(1)> No accessories';
    ...
    assert.equal(await page.locator('#lotRemark').inputValue(), note);
    assert.ok((await page.locator('#lotRemarkHelp').innerText()).includes('in English'));
    assert.equal(await page.locator('#lotRemark').getAttribute('aria-invalid'), null);
    ...
    await page.locator('#publishLot').click();
    await page.locator('#lotPage > .shared-report .shared-remark').waitFor({ state: 'visible' });
    assert.equal(await page.locator('#publishedLotRemarkText').textContent(), note);
    assert.equal(await page.locator('#lotPage > .shared-report .shared-remark img').count(), 0);
    assert.ok((await page.locator('#lotPage > .shared-report .shared-remark').innerText()).includes(note));
    ...
    assert.equal(await page.locator('#loadingOverlay').isVisible(), false);
    assert.equal(await page.locator('#lotRemark').getAttribute('aria-invalid'), 'true');
    assert.ok((await page.locator('#toast').innerText()).includes('英文'));
    ...
    await page.locator('#publishLot').click();
    await page.locator('#lotPage > .shared-report .shared-remark').waitFor({ state: 'visible' });
    assert.equal((await page.locator('#publishedLotRemarkText').textContent()).length, 1000);
    ...
    const headingBox = await page.locator('#auctionPage > .shared-report .shared-report-heading').boundingBox();
    const fieldsBox = await page.locator('#auctionPage > .shared-report dl').boundingBox();
    assert.ok(headingBox.y + headingBox.height <= remarksBox.y);
    assert.ok(remarksBox.y + remarksBox.height <= fieldsBox.y);
    assert.equal(await page.locator('#auctionRemarks').evaluate(element => element.parentElement.classList.contains('inspection')), true);
    assert.equal(await page.locator('#auctionRemarksText').evaluate(element => getComputedStyle(element).fontWeight), '700');
    await page.evaluate(() => setLanguage('zh'));
    assert.equal(await page.locator('#auctionRemarksTitle').innerText(), '备注');
