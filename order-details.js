...
(function(){
 ...
 const originalReport=!isStore?page.querySelector('.inspection')?.outerHTML:'';
 ...
 const snapshots={
  ...
  'FSO-260820-0048':{privacy:['不通过','Failed'],returns:['运营未发起','Not initiated by operations']},
  'FSO-260827-0101':{lot:'FS-MY-260823-0197',source:['卖家接受预中标价','Seller accepted pre-winning price']}
 };
 ...
 function render(){
  ...
  const data=snapshots[selected]||{},pending=(card.dataset.orderCard||card.dataset.orderStatus)==='pending';
  const row=(label,value)=>'<div class="summary-row"><span>'+esc(label)+'</span><strong>'+esc(value)+'</strong></div>';
  const owner=card.dataset.owner==='jia'||selected!=='FSO-260820-0048'?t('贾瑞真','Jia Ruizhen'):'Aina Rahman';
  ...
  page.innerHTML='<header class="topbar"><button class="icon-button" type="button" id="orderDetailBack" aria-label="'+t('返回订单','Back to orders')+'">‹</button><h1>'+t('订单详情','Order Details')+'</h1><span></span></header>'+
   '<section class="section"><h2 class="section-title">'+esc(card.querySelector('.unified-order-head strong').textContent)+'</h2>'+row(t('订单号','Order number'),selected)+row(t('商家名称','Merchant'),'FS Retail Malaysia')+row(t('店铺名称','Store'),t('中心旗舰店','Central Flagship Store'))+row(t('实际建拍操作人','Actual listing operator'),owner)+'</section>'+
   ...
   '<section class="section"><h2 class="section-title">'+t('取货结果','Pickup result')+'</h2>'+row(t('取货状态','Pickup status'),pending?t('待取货','Pending Pickup'):t('已取货','Picked Up'))+row(t('实际取货时间','Actual pickup time'),pending?'—':card.querySelector('.unified-pickup-time strong').textContent)+'<p class="small muted">'+t('取货结果由平台督导维护，本页面只读。','Pickup is recorded by the platform supervisor. This page is read-only.')+'</p></section>'+
  ...
  if(!isStore){
   if(selected==='FSO-260821-0098'&&originalReport){const box=document.createElement('div');box.innerHTML=originalReport;{const walker=document.createTreeWalker(box,NodeFilter.SHOW_TEXT);while(walker.nextNode()){const n=walker.currentNode,key=n.textContent.trim();if(key&&typeof zh!=='undefined'){const english=Object.keys(zh).find(k=>zh[k]===key)||key;n.textContent=n.textContent.replace(key,en?english:(zh[english]||key));}}}page.append(...box.childNodes);}
   else page.insertAdjacentHTML('beforeend','<section class="section"><h2>'+t('验机报告','Inspection Report')+'</h2><p class="small muted">'+t('暂无该订单的质检快照','No inspection snapshot available for this order')+'</p></section>');
  }
