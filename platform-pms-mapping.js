...
function mount(win){
 ...
 function extraEditor(code,rowIndex){const x=catalogItems(state).find(x=>x.code===code);if(code&&!x)throw Error('附加属性值已删除，请刷新。');extraEditing={code,revision:x?.revision,rowIndex};extraDialog.innerHTML=`<header><h2>${x?'编辑':'新增'}附加属性值</h2><button class="pms-close" data-extra-action="close" aria-label="关闭">×</button></header><div class="pms-dialog-body">${field('二级属性项 *',`<select id="exAttribute" class="control" ${rowIndex!==undefined||(x&&extraReferences(state,code).length)?'disabled':''}>${options(attributes.map(a=>({id:a.id,name:catName(a.category)+' / '+a.name})),rowIndex!==undefined?editor.draft.sourcePpn:x?extraAttribute(state,x):'','请选择二级属性项')}</select>`)}${field('附加属性值 *',input('exName',x?.name||'','text','placeholder="例如：马来版" maxlength="100"'))}<p id="pmsExtraError" class="pms-error" role="alert" hidden></p></div><footer><button class="btn" data-extra-action="close">取消</button><button class="btn primary" data-extra-action="save">保存</button></footer>`;extraDialog.showModal();$('#exName').disabled=!$('#exAttribute').value;($('#exAttribute').value?$('#exName'):$('#exAttribute')).focus();}
 extraDialog.addEventListener('click',e=>{const action=e.target.closest('[data-extra-action]')?.dataset.extraAction;if(!action)return;if(action==='close'){extraDialog.close();return;}try{permission(access);refresh();const x=saveExtra(state,{sourcePpn:$('#exAttribute').value,name:$('#exName').value},extraEditing.code,extraEditing.revision,access);persist();extraDialog.close();if(extraEditing.rowIndex!==undefined&&dialog.open){const d=editor.draft.details[extraEditing.rowIndex];d.code=x.code;d.name=x.name;renderDetails();}else render();}catch(err){$('#pmsExtraError').textContent=err.message;$('#pmsExtraError').hidden=false;}});
 ...
 function openEditor(id){
  ...
  <section class="pms-editor-section"><h3>3. 映射关系</h3><div id="pmTargetField"></div><div id="pmDetailRows"></div><div id="pmAddDetail">${button('detail-add','＋ 添加映射')}</div></section>
  <section class="pms-editor-section"><h3>生效设置</h3><div class="pms-form">${field('状态',`<select id="pmEnabled" class="control">${option('enabled','启用',r&&status(v)==='启用')}${option('disabled','停用',!r||status(v)!=='启用')}</select>`)}</div></section>`,button('save','保存','','primary'));
 ...
 function renderAttributes(){const v=editor.draft,attrs=attributes.filter(a=>a.category===v.category),a=attr(v.sourcePpn),required=source(v.triggerPpv)?.requiresConfirmation;if(required)editor.direct=false;$('#pmAttributeFields').innerHTML=`<div class="pms-form">${field('二级属性项 *',`<select id="pmAttribute" class="control">${options(attrs,v.sourcePpn,'请选择属性项')}</select>`)}${field('触发属性值 *',`<select id="pmTrigger" class="control">${options(a?.values||[],v.triggerPpv,'请选择属性值')}</select>`)}${field('处理方式',`<select id="pmConfirmMode" class="control"><option value="extra" ${!editor.direct?'selected':''}>店员补充选择后映射</option><option value="direct" ${editor.direct?'selected':''} ${required?'disabled':''}>直接映射</option></select>`)}${field('优先级 *',input('pmPriority',v.priority,'number','min="1" step="1"')+'<small>数字越小越优先。</small>')}</div>`;}
 ...
 function renderDetails(){const v=editor.draft,targets=attr(v.targetPpn)?.targets||[];$('#pmTargetField').innerHTML=field('一级取价映射属性项 *',`<select id="pmTargetAttribute" class="control">${options(attributes.filter(a=>a.category===v.category).map(a=>({id:a.targetId,name:a.name})),v.targetPpn,'请选择一级属性项')}</select>`);$('#pmDetailRows').innerHTML=`<table class="table pms-detail-table"><thead><tr>${editor.direct?'':'<th>店员可选的附加属性值</th>'}<th>一级取价映射值 *</th>${editor.direct?'':'<th></th>'}</tr></thead><tbody>${v.details.map((d,i)=>`<tr data-detail-row="${i}">${editor.direct?'':`<td><select class="control" data-detail-extra>${options(catalogItems(state).filter(x=>extraAttribute(state,x)===v.sourcePpn).map(x=>({id:x.code,name:x.name})),d.code,'请选择附加属性值')}</select><button class="btn link" data-pms="extra-inline-new" data-id="${i}" ${!v.sourcePpn?'disabled':''}>＋ 新增属性值</button></td>`}<td><select class="control" data-detail-target ${!v.targetPpn?'disabled':''}>${options(targets,d.targetPpv,v.targetPpn?'请选择一级属性值':'请先选择一级属性项')}</select></td>${editor.direct?'':`<td>${button('detail-remove','移除',String(i),'link')}</td>`}</tr>`).join('')}</tbody></table>`;$('#pmAddDetail').hidden=editor.direct;}
 function syncScopes(){$('#pmModelChoices').hidden=$('#pmModelMode').value==='all';}
 function captureDraft(){const v=editor.draft,cat=category($('#pmCategory').value);Object.assign(v,{country:'MY',category:cat?.id||'',sourceStandard:cat?.source||'',targetStandard:cat?.target||'',models:$('#pmModelMode').value==='all'?[]:$$('#pmModels input:checked').map(x=>x.value),sourcePpn:$('#pmAttribute').value,triggerPpv:$('#pmTrigger').value,targetPpn:$('#pmTargetAttribute').value,details:readDetails(),priority:Number($('#pmPriority').value),reason:editor.id?'编辑配置':'新建配置'});return v;}
 ...
 function act(e){const b=e.target.closest('[data-pms]');if(!b)return;const action=b.dataset.pms,id=b.dataset.id;try{
  ...
  permission(access);refresh();if(action==='extra-new'||action==='extra-edit'){extraEditor(id||undefined);return;}if(action==='extra-delete'){extraDelete(id);return;}if(action==='extra-delete-confirm'){deleteExtra(state,editor.extraCode,editor.extraRevision,access);persist();dialog.close();render();return;}if(action==='extra-inline-new'){captureDraft();extraEditor(undefined,Number(id));return;}if(action==='new'||action==='edit'){openEditor(id);return;}
 ...
 if(id==='pmAttribute'||id==='pmTrigger'){captureDraft();if(id==='pmAttribute'){editor.draft.triggerPpv='';editor.draft.targetPpn='';}editor.draft.details=[blankDetail()];editor.direct=false;renderAttributes();renderDetails();}
