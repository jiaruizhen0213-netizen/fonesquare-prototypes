...
const mappingGroups=v=>v.mappingGroups||[{triggerPpv:v.triggerPpv,details:v.details}];
const expandGroups=v=>mappingGroups(v).map(g=>{const x={...v,...g};delete x.mappingGroups;return x;});
const allDetails=v=>mappingGroups(v).flatMap(g=>g.details);
...
function extraAttribute(s,x){if(attributes.some(a=>a.id===x.sourcePpn))return x.sourcePpn;const refs=[...new Set(s.rules.flatMap(r=>r.versions).filter(v=>allDetails(v).some(d=>d.code===x.code)).map(v=>v.sourcePpn))];return refs.length===1?refs[0]:'';}
function upgradeCatalog(s){for(const x of s.extraCatalog){if(!x.name){const d=s.rules.flatMap(r=>r.versions).reverse().flatMap(expandGroups).filter(v=>sameBase(v,x)).flatMap(v=>v.details).find(d=>d.code===x.code);x.name=d?.name||x.code;}x.revision=x.revision||1;x.updatedAt=x.updatedAt||'2026-09-14T07:00:00Z';x.operator=x.operator||'贾瑞真';}return s;}
function extraReferences(s,code){return s.rules.flatMap(r=>r.versions.filter(v=>allDetails(v).some(d=>d.code===code)).map(v=>({id:r.id,version:v.version,name:v.name,status:status(v)})));}
...
function validate(s,v,enabling=false,excludeId='',at=Date.now()){
 if(v.mappingGroups){if(!v.mappingGroups.length)return '请至少添加一组映射。';const seen=new Set();for(const g of expandGroups(v)){if(seen.has(g.triggerPpv))return '同一规则内的触发属性值不能重复，请在同一组中添加附加值映射。';seen.add(g.triggerPpv);const error=validate(s,g,enabling,excludeId,at);if(error)return error;}return '';}
 ...
 if(enabling){
  ...
  const conflicts=[];
  s.rules.filter(r=>r.id!==excludeId).forEach(r=>{const w=active(r,at);if(w&&expandGroups(w).some(g=>sameBase(g,v))&&w.priority===v.priority&&overlap(v.models,w.models))conflicts.push(`${r.id} / V${w.version}（型号 ${w.models.map(id=>model(id)?.name||id).join('、')||'本品类全部'}）`);});
...
function saveRule(s,draft,id,expectedRevision,p='maintain',at=new Date().toISOString()){
 ...
 upgradeCatalog(s);const v=clone(draft);['channels','start','end','prompt','desiredStatus'].forEach(k=>delete v[k]);allDetails(v).forEach(d=>d.models=[]);allDetails(v).forEach(d=>{if(d.code){const x=catalogItems(s).find(x=>x.code===d.code);if(x)d.name=x.name;}});const error=validate(s,v);if(error)throw Error(error);
 allDetails(v).forEach(d=>{d.targetName=target(d.targetPpv).name;});
 if(v.mappingGroups){v.mappingGroups.forEach(g=>g.sourceName=source(g.triggerPpv).name);v.triggerPpv=v.mappingGroups[0].triggerPpv;v.details=clone(v.mappingGroups[0].details);}
...
function registerCoverage(s,v,at){
 if(v.mappingGroups){expandGroups(v).forEach(g=>registerCoverage(s,g,at));return;}
...
function snapshot(s,context,draft=null){
 const rules=s.rules.flatMap(r=>{const v=active(r,instant(context.at));return v?expandGroups(clone(v)).map(g=>({...g,id:r.id})):[];});
 if(draft){const v=clone(draft);rules.splice(0,rules.length,...rules.filter(r=>r.id!==v.id));['disabledAt','replacedAt','activatedAt'].forEach(k=>delete v[k]);rules.push(...expandGroups(v).map(g=>({...g,enabled:true})));}
 ...
 return {id:`SNAP-${s.revision}`,at:context.at,context:inputs,rules,coverage,ruleVersions:[...new Set(rules.map(v=>`${v.id} / V${v.version}`))]};
}
...
function mount(win){
 ...
 function detailsHtml(v){if(v.mappingGroups)return expandGroups(v).map(g=>`<div><b>${esc(pvName(g.triggerPpv))}</b>${detailsHtml(g)}</div>`).join('');return `<div class="pms-pairs">${v.details.map(d=>`<div><span>${esc(d.name||'直接映射')}<small>${esc(d.code||'无需附加确认')}</small></span><span class="pms-arrow">→</span><span>${esc(d.targetName||targetName(d.targetPpv))}<small>${esc(d.targetPpv)}</small></span></div>`).join('')}</div>`;}
 function render(){
 ...
 function openEditor(id){
  ...
  v.mappingGroups=clone(mappingGroups(v));editor={id:r?.id,revision:r?.revision,draft:v,direct:r?!allDetails(v).some(d=>d.code||d.name):false};
  ...
  <section class="pms-editor-section"><h3>3. 映射关系</h3><div id="pmDetailRows"></div><div id="pmAddDetail">${button('group-add','＋ 添加映射')}</div></section>
 ...
 function renderAttributes(){const v=editor.draft,attrs=attributes.filter(a=>a.category===v.category),required=mappingGroups(v).some(g=>source(g.triggerPpv)?.requiresConfirmation);if(required)editor.direct=false;$('#pmAttributeFields').innerHTML=`<div class="pms-form">${field('属性项 *',`<select id="pmAttribute" class="control">${options(attrs,v.sourcePpn,'请选择属性项')}</select>`)}${field('处理方式',`<select id="pmConfirmMode" class="control"><option value="extra" ${!editor.direct?'selected':''}>店员补充选择后映射</option><option value="direct" ${editor.direct?'selected':''} ${required?'disabled':''}>直接映射</option></select>`)}</div>`;}
 function readGroups(){return $$('#pmDetailRows [data-mapping-group]').map(block=>({triggerPpv:block.querySelector('[data-group-trigger]').value,details:[...block.querySelectorAll('[data-detail-row]')].map(row=>({code:editor.direct?'':row.querySelector('[data-detail-extra]').value,name:editor.direct?'':catalogItems(state).find(x=>x.code===row.querySelector('[data-detail-extra]').value)?.name||'',targetPpv:row.querySelector('[data-detail-target]').value,models:[]}))}));}
 ...
 function renderDetails(){const v=editor.draft,targets=attr(v.targetPpn)?.targets||[];
 $('#pmDetailRows').innerHTML=v.mappingGroups.map((g,gi)=>{const triggerCell=`<td data-detail-trigger rowspan="${Math.max(1,g.details.length)}"><select ${gi===0?'id="pmTrigger"':''} data-group-trigger="${gi}" class="control" aria-label="触发属性值" ${!v.sourcePpn?'disabled':''}>${options(attr(v.sourcePpn)?.values||[],g.triggerPpv,'请选择触发属性值')}</select></td>`;return `<div class="pms-version" data-mapping-group="${gi}"><table class="table pms-detail-table"><thead><tr><th>触发属性值</th><th>店员可选的附加属性值</th><th>一级取价映射值 *</th><th></th></tr></thead><tbody>${g.details.map((d,i)=>`<tr data-detail-row="${i}">${i===0?triggerCell:''}${editor.direct?'<td class="subtle">无需补充</td>':`<td><select class="control" data-detail-extra>${options(catalogItems(state).filter(x=>extraAttribute(state,x)===v.sourcePpn).map(x=>({id:x.code,name:x.name})),d.code,'请选择附加属性值')}</select></td>`}<td><select class="control" data-detail-target ${!v.targetPpn?'disabled':''}>${options(targets,d.targetPpv,v.targetPpn?'请选择一级属性值':'请先选择属性项')}</select></td><td>${editor.direct?'':button('detail-remove','移除',gi+':'+i,'link')}</td></tr>`).join('')||`<tr>${triggerCell}<td colspan="3" class="subtle">请添加附加值映射</td></tr>`}</tbody></table><div class="pms-actions">${editor.direct?'':button('detail-add','＋ 添加附加值映射',String(gi))}${button('group-remove','移除整组',String(gi),'link')}</div></div>`;}).join('');
 }
 function syncScopes(){$('#pmModelChoices').hidden=$('#pmModelMode').value==='all';}
 function captureDraft(){const v=editor.draft,cat=category($('#pmCategory').value);Object.assign(v,{country:'MY',category:cat?.id||'',sourceStandard:cat?.source||'',targetStandard:cat?.target||'',models:$('#pmModelMode').value==='all'?[]:$$('#pmModels input:checked').map(x=>x.value),sourcePpn:$('#pmAttribute').value,mappingGroups:readGroups(),priority:Number($('#pmPriority').value),reason:editor.id?'编辑配置':'新建配置'});v.triggerPpv=v.mappingGroups[0]?.triggerPpv||'';v.details=v.mappingGroups[0]?.details||[];return v;}
 function history(id){const r=state.rules.find(r=>r.id===id);show(id+' · 历史版本',`<p class="subtle">创建人 ${esc(r.createdBy)} · ${time(r.createdAt)}</p>`+r.versions.slice().reverse().map(v=>`<section class="pms-version"><h3>V${v.version} ${pill(status(v))}</h3><div class="pms-summary"><span>品类：${catName(v.category)}</span><span>国家：马来西亚</span><span>型号：${esc(v.models.map(name).join('、')||'本品类全部')}</span><span>属性项：${esc(attr(v.sourcePpn)?.name)} / ${esc(v.sourcePpn)}</span><span>触发属性值：${esc(mappingGroups(v).map(g=>pvName(g.triggerPpv)).join('、'))}</span><span>优先级：${v.priority}</span><span>更新：${esc(v.operator)} · ${time(v.updatedAt)}</span></div>${detailsHtml(v)}</section>`).join('')+'<h3>操作记录</h3>'+state.audit.filter(a=>a.id===id).map(a=>`<p>${time(a.at)} · ${esc(a.operator)} · ${esc(a.action)}</p>`).join(''));}
 ...
 function act(e){const b=e.target.closest('[data-pms]');if(!b)return;const action=b.dataset.pms,id=b.dataset.id;try{
  ...
  permission(access);refresh();if(action==='extra-new'||action==='extra-edit'){extraEditor(id||undefined);return;}if(action==='extra-delete'){extraDelete(id);return;}if(action==='extra-delete-confirm'){deleteExtra(state,editor.extraCode,editor.extraRevision,access);persist();dialog.close();render();return;}if(action==='new'||action==='edit'){openEditor(id);return;}
  if(action==='group-add'){captureDraft();editor.draft.mappingGroups.push({triggerPpv:'',details:[blankDetail()]});renderDetails();return;}
  if(action==='group-remove'){captureDraft();editor.draft.mappingGroups.splice(Number(id),1);renderAttributes();renderDetails();return;}
  if(action==='detail-add'){captureDraft();editor.draft.mappingGroups[Number(id)].details.push(blankDetail());renderDetails();return;}
  if(action==='detail-remove'){captureDraft();const [g,i]=id.split(':').map(Number);editor.draft.mappingGroups[g].details.splice(i,1);renderDetails();return;}
 ...
 dialog.addEventListener('change',e=>{const id=e.target.id;if(id==='pmCategory'){captureDraft();Object.assign(editor.draft,{models:[],sourcePpn:'',triggerPpv:'',targetPpn:'',mappingGroups:[{triggerPpv:'',details:[blankDetail()]}]});editor.direct=false;$('#pmModelMode').value='all';renderModels();renderAttributes();renderDetails();syncScopes();}
 if(id==='pmAttribute'){captureDraft();editor.draft.targetPpn=attr(editor.draft.sourcePpn)?.targetId||'';editor.draft.mappingGroups=[{triggerPpv:'',details:[blankDetail()]}];editor.direct=false;renderAttributes();renderDetails();}
 if(e.target.matches('[data-group-trigger]')){captureDraft();if(editor.direct&&mappingGroups(editor.draft).some(g=>source(g.triggerPpv)?.requiresConfirmation)){editor.direct=false;}renderAttributes();renderDetails();}
 if(id==='pmConfirmMode'){captureDraft();editor.direct=e.target.value==='direct'&&!mappingGroups(editor.draft).some(g=>source(g.triggerPpv)?.requiresConfirmation);editor.draft.mappingGroups.forEach(g=>g.details=[blankDetail()]);renderAttributes();renderDetails();}
 if(id==='pmModelMode'){captureDraft();syncScopes();renderDetails();}
 if(e.target.closest('#pmModels')){captureDraft();renderDetails();}
