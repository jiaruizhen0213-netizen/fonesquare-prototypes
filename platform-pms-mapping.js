...
function mount(win){
 ...
 function openEditor(id){
  ...
  <section class="pms-editor-section"><h3>3. 映射关系</h3><div id="pmDetailRows"></div><div id="pmAddDetail">${button('detail-add','＋ 添加映射')}</div></section>
 ...
 function renderDetails(){const v=editor.draft,targets=attr(v.targetPpn)?.targets||[];$('#pmDetailRows').innerHTML=`<table class="table pms-detail-table"><thead><tr><th>触发属性值</th>${editor.direct?'':'<th>店员可选的附加属性值</th>'}<th>一级取价映射值 *</th>${editor.direct?'':'<th></th>'}</tr></thead><tbody>${v.details.map((d,i)=>`<tr data-detail-row="${i}"><td data-detail-trigger>${esc(source(v.triggerPpv)?.name||'—')}</td>${editor.direct?'':`<td><select class="control" data-detail-extra>${options(catalogItems(state).filter(x=>extraAttribute(state,x)===v.sourcePpn).map(x=>({id:x.code,name:x.name})),d.code,'请选择附加属性值')}</select><button class="btn link" data-pms="extra-inline-new" data-id="${i}" ${!v.sourcePpn?'disabled':''}>＋ 新增属性值</button></td>`}<td><select class="control" data-detail-target ${!v.targetPpn?'disabled':''}>${options(targets,d.targetPpv,v.targetPpn?'请选择一级属性值':'请先选择属性项')}</select></td>${editor.direct?'':`<td>${button('detail-remove','移除',String(i),'link')}</td>`}</tr>`).join('')}</tbody></table>`;$('#pmAddDetail').hidden=editor.direct;}
