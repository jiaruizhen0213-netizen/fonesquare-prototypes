...
function mount(win){
 ...
 function history(id){const r=state.rules.find(r=>r.id===id);show(id+' · 历史版本',`<p class="subtle">创建人 ${esc(r.createdBy)} · ${time(r.createdAt)}</p>`+r.versions.slice().reverse().map(v=>`<section class="pms-version"><h3>V${v.version} ${pill(status(v))}</h3><div class="pms-summary"><span>品类：${catName(v.category)}</span><span>型号：${esc(v.models.map(name).join('、')||'本品类全部')}</span><span>属性项：${esc(attr(v.sourcePpn)?.name)} / ${esc(v.sourcePpn)}</span><span>触发属性值：${esc(mappingGroups(v).map(g=>pvName(g.triggerPpv)).join('、'))}</span><span>优先级：${v.priority}</span><span>更新：${esc(v.operator)} · ${time(v.updatedAt)}</span></div>${detailsHtml(v)}</section>`).join('')+'<h3>操作记录</h3>'+state.audit.filter(a=>a.id===id).map(a=>`<p>${time(a.at)} · ${esc(a.operator)} · ${esc(a.action)}</p>`).join(''));}
