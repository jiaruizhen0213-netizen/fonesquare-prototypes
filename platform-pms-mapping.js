...
function validate(s,v,enabling=false,excludeId='',at=Date.now()){
 ...
 if(enabling){
  ...
  const conflicts=[];
  s.rules.filter(r=>r.id!==excludeId).forEach(r=>r.versions.forEach(w=>{if(w.enabled&&sameBase(w,v)&&w.priority===v.priority&&overlap(v.models,w.models)&&Math.max(instant(v.start),at)<ends(w)&&begins(w)<instant(v.end))conflicts.push(`${r.id} / V${w.version}（型号 ${w.models.map(id=>model(id)?.name||id).join('、')||'本品类全部'}；${w.start} 至 ${w.end||'长期'}）`);}));
...
function saveRule(s,draft,id,expectedRevision,p='maintain',at=new Date().toISOString()){
 ...
 const v=clone(draft);delete v.channels;const error=validate(s,v);if(error)throw Error(error);
...
function mount(win){
 ...
 function detailsHtml(v){return `<div class="pms-pairs">${v.details.map(d=>`<div><span>${esc(d.name||'直接映射')}<small>${esc(d.code||'无需附加确认')} · ${esc(d.models.length?d.models.map(name).join('、'):'全部支持型号')}</small></span><span class="pms-arrow">→</span><span>${esc(d.targetName||targetName(d.targetPpv))}<small>${esc(d.targetPpv)}</small></span></div>`).join('')}</div>`;}
 function render(){
 ...
 function openEditor(id){
  ...
  ${field('国家／站点 *','<select id="pmCountry" class="control"><option value="MY">马来西亚</option></select>',true)}
 ...
 function readDetails(){return $$('#pmDetailRows [data-detail-row]').map((row,i)=>({code:editor.draft.details[i]?.code||'',name:editor.direct?'':row.querySelector('[data-detail-name]').value.trim(),targetPpv:row.querySelector('[data-detail-target]').value,models:[...row.querySelector('[data-detail-models]').selectedOptions].map(o=>o.value).filter(id=>id!=='__all')}));}
 ...
 function history(id){const r=state.rules.find(r=>r.id===id);show(id+' · 历史版本',`<p class="subtle">创建人 ${esc(r.createdBy)} · ${time(r.createdAt)}</p>`+r.versions.slice().reverse().map(v=>`<section class="pms-version"><h3>V${v.version} ${pill(status(v))}</h3><div class="pms-summary"><span>名称：${esc(v.name)}</span><span>品类：${catName(v.category)}</span><span>国家：马来西亚</span><span>型号：${esc(v.models.map(name).join('、')||'本品类全部')}</span><span>属性项：${esc(attr(v.sourcePpn)?.name)} / ${esc(v.sourcePpn)}</span><span>触发属性值：${esc(v.sourceName||pvName(v.triggerPpv))}</span><span>优先级：${v.priority}</span><span>有效时间：${time(v.start)} 至 ${time(v.end)}</span><span>确认提示：${esc(v.prompt)}</span><span>原因：${esc(v.reason)} · ${esc(v.operator)} · ${time(v.updatedAt)}</span></div>${detailsHtml(v)}</section>`).join('')+'<h3>操作记录</h3>'+state.audit.filter(a=>a.id===id).map(a=>`<p>${time(a.at)} · ${esc(a.operator)} · ${esc(a.action)} · ${esc(a.reason)}</p>`).join(''));}
 ...
 function preview(id){const r=state.rules.find(r=>r.id===id),v=r?{...clone(latest(r)),id}:null;editor={previewDraft:v,previewCategory:v?.category||'PHONE'};show(v?`${id} / V${v.version} · 版本预览`:'统一命中预览',`<p class="pms-note">先命中整条规则，再选择附加值并查看对应取价目标。${v?'本次临时使用所选版本，草稿不参与实际处理。':''}预览不创建商品码，不修改真实结果或价格。</p><div class="pms-form">${field('国家','<select id="pvCountry" class="control"><option value="MY">马来西亚</option></select>')}${field('品类',`<select id="pvCategory" class="control">${options(categories,editor.previewCategory)}</select>`)}${field('型号','<select id="pvModel" class="control"></select>')}${field('执行时间（MYT）',input('pvTime',local(new Date().toISOString()),'datetime-local','step="1"'),true)}</div><div id="pvGroups"></div><div id="pvOutput" class="pms-preview"></div>`,button('preview-run','执行预览','','primary'));$('#pvTime').dataset.initial=$('#pvTime').value;previewCategory();}
 ...
 function previewContext(){const entered=$('#pvTime').value;return {category:$('#pvCategory').value,country:$('#pvCountry').value,model:$('#pvModel').value,at:entered===$('#pvTime').dataset.initial?new Date().toISOString():fromLocal(entered),inputs:{actual:[],baseline:[]}};}
 ...
 function act(e){const b=e.target.closest('[data-pms]');if(!b)return;const action=b.dataset.pms,id=b.dataset.id;try{
  if(action==='close'){dialog.close();return;}if(action==='tab'){tab=id;render();return;}if(action==='search'){readFilters();renderRows();return;}if(action==='reset'){filter={keyword:'',category:'',model:'',trigger:'',extra:'',status:''};render();return;}
  ...
  if(action==='save'){const d=captureDraft();if($('#pmModelMode').value==='selected'&&!d.models.length)throw Error('请选择至少一个型号。');saveRule(state,d,editor.id,editor.revision,access);persist();dialog.close();render();notify('已保存整条规则草稿，启用后生效');return;}
 ...
 if(e.target.matches('[data-detail-models]')){const el=e.target,chosen=[...el.selectedOptions].map(o=>o.value);if(!chosen.length||(chosen.includes('__all')&&el.dataset.selection!=='all')){for(const o of el.options)o.selected=o.value==='__all';}else if(chosen.length>1){el.querySelector('option[value="__all"]').selected=false;}el.dataset.selection=el.selectedOptions[0]?.value==='__all'?'all':'selected';}
 ...
 if(id==='pvCategory')previewCategory();else if(['pvModel','pvTime','pv-actual','pv-baseline'].includes(id))previewExtras();else if(e.target.matches('[data-preview-extra]'))$('#pvOutput').innerHTML='';
 });
...
return {KEY,LEGACY_KEY,categories,models,attributes,source,target,category,clone,seed,status,active,latest,validate,saveRule,enableRule,disableRule,snapshot,resolve,confirmation,evaluate,retryRecord,reprocess,mount};
});
