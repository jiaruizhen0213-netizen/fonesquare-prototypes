...
function tableOf(rows,legacy=false){
 ...
 return `<div class="table-wrap"><table class="history-table"><thead><tr>${range?'<th>开始时间比例（%）</th><th>结束时间比例（%）</th>':`<th>${legacy?'原剩余时间（分钟）':'原竞拍剩余时间比例（单值）'}</th>`}<th>${legacy?'原出价比例':'首次出价比例'}</th>${legacy?'':'<th>出价次数</th>'}</tr></thead><tbody>${rows.map(r=>`<tr>${range?`<td>${pct(r.startRemainingPercent)}</td><td>${pct(r.endRemainingPercent)}</td>`:`<td>${legacy?esc(r.minutes):pct(r.remainingPercent)}</td>`}<td>${esc(legacy?r.percent:r.firstBidPercent)}%</td>${legacy?'':`<td>${esc(r.bidCount)} 次</td>`}</tr>`).join('')}</tbody></table></div>`;
}
...
function render(){
 ...
 <p class="range-help">竞拍剩余时间比例＝剩余时间 ÷ 本轮竞拍总时长；区间按开始比例 ≥ 剩余比例 > 结束比例匹配。</p>${draft.rows.some(r=>r.endRemainingPercent===null)?'<p class="range-help">请补充结束时间比例后保存；旧版单值配置未自动推算区间。</p>':''}<div class="error" id="error" role="alert"></div>
