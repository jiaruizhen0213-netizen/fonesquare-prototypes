...
function renderRows(){
 $('#rows').innerHTML=draft.rows.map((r,i)=>`<tr data-row="${i}"><td class="row-number">${i+1}</td><td>${input('remainingPercent',r.remainingPercent,i)}</td><td>${input('firstBidPercent',r.firstBidPercent,i)}</td><td>${input('bidCount',r.bidCount,i)}</td><td class="preview-amount">${preview(r)}</td><td><button class="delete" data-action="remove" data-index="${i}" ${draft.rows.length===1||!canEdit?'disabled':''}>删除</button></td></tr>`).join('');
}
...
function render(){
 ...
 <div class="table-wrap"><table class="config-table"><colgroup><col style="width:7%"><col style="width:22%"><col style="width:22%"><col style="width:20%"><col style="width:21%"><col style="width:8%"></colgroup><thead><tr><th>序号</th><th>竞拍剩余时间比例（%）<small>剩余时间 ÷ 本轮竞拍总时长</small></th><th>首次出价比例（%）<small>首次报价占自营最高出价的比例</small></th><th>出价次数<small>包含首次出价的总次数</small></th><th>预计首次报价</th><th>操作</th></tr></thead><tbody id="rows"></tbody></table></div>
