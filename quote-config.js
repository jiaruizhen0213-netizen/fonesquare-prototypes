...
try{const saved=JSON.parse(localStorage.getItem(KEY));state=saved?.schema===2&&saved.current?saved:C.seed(JSON.parse(localStorage.getItem(LEGACY_KEY)));}catch(_){state=C.seed();}
let draft=C.clone(state.current),dirty=false;
...
function readRows(){return [...document.querySelectorAll('[data-row]')].map(row=>Object.fromEntries(['remainingPercent','firstBidPercent','bidCount'].map(k=>{const value=row.querySelector(`[data-field="${k}"]`).value;return [k,value===''?null:Number(value)];})));}
...
function input(key,value,i){const count=key==='bidCount',labels={remainingPercent:'竞拍剩余时间比例',firstBidPercent:'首次出价比例',bidCount:'出价次数'};return `<div class="input-unit"><input aria-label="第${i+1}行${labels[key]}" data-field="${key}" type="number" min="${count?'1':'0'}" ${count?'step="1"':'max="100" step="any"'} value="${Number.isFinite(value)?value:''}" placeholder="${count?'输入次数':'输入比例'}" ${!canEdit?'disabled':''}><span>${count?'次':'%'}</span></div>`;}
function renderRows(){
 $('#rows').innerHTML=draft.rows.map((r,i)=>`<tr data-row="${i}"><td class="row-number">${i+1}</td><td>${input('remainingPercent',r.remainingPercent,i)}</td><td>${input('firstBidPercent',r.firstBidPercent,i)}</td><td>${input('bidCount',r.bidCount,i)}</td><td><button class="delete" data-action="remove" data-index="${i}" ${draft.rows.length===1||!canEdit?'disabled':''}>删除</button></td></tr>`).join('');
}
function tableOf(rows,legacy=false){return `<div class="table-wrap"><table class="history-table"><thead><tr><th>${legacy?'原剩余时间（分钟）':'竞拍剩余时间比例'}</th><th>${legacy?'原出价比例':'首次出价比例'}</th>${legacy?'':'<th>出价次数</th>'}</tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(legacy?r.minutes:r.remainingPercent)}${legacy?'':'%'}</td><td>${esc(legacy?r.percent:r.firstBidPercent)}%</td>${legacy?'':`<td>${esc(r.bidCount)} 次</td>`}</tr>`).join('')}</tbody></table></div>`;}
...
function render(){
 ...
 <div class="config-meta"><label class="field">状态<select id="enabled" ${!canEdit?'disabled':''}><option value="true" ${draft.enabled?'selected':''}>启用</option><option value="false" ${!draft.enabled?'selected':''}>停用</option></select></label><button class="link" data-action="history">${historyOpen?'收起历史版本':'历史版本'}</button></div>
 <div class="error" id="error" role="alert"></div>
 <div class="table-wrap"><table class="config-table"><colgroup><col style="width:7%"><col style="width:29%"><col style="width:29%"><col style="width:27%"><col style="width:8%"></colgroup><thead><tr><th>序号</th><th>竞拍剩余时间比例（%）<small>剩余时间 ÷ 本轮竞拍总时长</small></th><th>首次出价比例（%）<small>首次报价占自营最高出价的比例</small></th><th>出价次数<small>包含首次出价的总次数</small></th><th>操作</th></tr></thead><tbody id="rows"></tbody></table></div>
...
document.addEventListener('click',e=>{const b=e.target.closest('[data-action]');if(!b||b.disabled)return;const a=b.dataset.action;
 ...
 if(!canEdit)return;
 try{
  if(a==='add'){capture();draft.rows.push({remainingPercent:null,firstBidPercent:null,bidCount:null});renderRows();markDirty();}
...
document.addEventListener('input',e=>{if(canEdit&&e.target.matches('[data-field]')){capture();markDirty();$('#error').textContent='';}});
