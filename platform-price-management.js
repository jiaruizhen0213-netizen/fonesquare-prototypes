...
const headers=['型号编码','内存','卖场','币种','本地价格','价格来源'];
...
function validateImport(rows){
 const seen=new Set();
 return rows.map((r,i)=>{
  const modelId=String(r['型号编码']??'').trim(),model=models.find(m=>m.id===modelId),memory=normMemory(r['内存']),market=String(r['卖场']??'').trim(),currency=String(r['币种']??'').trim().toUpperCase(),raw=r['本地价格'],price=Number(raw),source=String(r['价格来源']??'').trim();
  ...
  return {row:r.__row||i+2,modelId,modelName:model?.name||'—',category:model?.category,memory,market,currency,price,source,errors};
 });
}
...
function blankPolicy(){return {category:'PHONE',modelMode:'all',modelIds:[],memoryMode:'all',memories:[],min:0,max:null,priority:10,rateOn:true,rate:0,fixedOn:false,fixed:0};}
function validatePolicy(p){
 ...
 if(!Number.isFinite(p.min)||p.min<0||p.max!==null&&(!Number.isFinite(p.max)||p.max<=p.min))return '价格区间须满足0≤下限＜上限；上限可留空。';
 if(!Number.isSafeInteger(p.priority)||p.priority<1)return '优先级须为正整数。';
 if(!p.rateOn&&!p.fixedOn)return '请至少选择一种调整方式。';
 if(p.rateOn&&(!Number.isFinite(p.rate)||p.rate< -100))return '比例调整须为有效数字，最低为-100%。';
 if(p.fixedOn&&!Number.isFinite(p.fixed))return '固定金额须为有效数字。';
 return '';
}
...
function conflict(a,b){return a.priority===b.priority&&Math.max(a.min,b.min)<Math.min(a.max??Infinity,b.max??Infinity)&&scopedModels(a).some(m=>m.memories.some(v=>matchesScope(a,m.id,v)&&matchesScope(b,m.id,v)));}
function savePolicy(state,draft,id,operator,time){
 ...
 const clean={category:draft.category,modelMode:draft.modelMode,modelIds:copy(draft.modelIds),memoryMode:draft.memoryMode,memories:copy(draft.memories),min:draft.min,max:draft.max,priority:draft.priority,rateOn:draft.rateOn,rate:draft.rate,fixedOn:draft.fixedOn,fixed:draft.fixed};
...
function calculate(state,{modelId,memory,actualP1,baselineP1}){
 ...
 const matched=state.policies.filter(q=>q.status==='启用'&&matchesScope(q,modelId,memory)&&itemPrice>=q.min&&(q.max===null||itemPrice<q.max)).sort((a,b)=>a.priority-b.priority);
 if(matched.length>1&&matched[0].priority===matched[1].priority)throw Error('命中同优先级的多条策略，请处理配置冲突。');const strategy=matched[0]||null;
 const rateAmount=strategy?.rateOn?itemPrice*strategy.rate/100:0,fixedAmount=strategy?.fixedOn?strategy.fixed:0,referencePrice=itemPrice+rateAmount+fixedAmount;
 if(!Number.isFinite(referencePrice)||referencePrice<=0)throw Error('策略计算结果无效，请检查调整比例和金额。');
 return {localPrice:p.price,batchId:p.batchId,coefficient,itemPrice,rateAmount,fixedAmount,referencePrice,strategy:strategy?copy(strategy):null};
}
function seed(){
 let s={revision:1,batchSerial:1,policySerial:1,prices:[],batches:[],policies:[]};
 s=importPrices(s,[{'型号编码':'103','内存':'256GB','卖场':'KL 示例卖场','币种':'MYR','本地价格':1000,'价格来源':'原型示例'},{'型号编码':'101','内存':'128GB','卖场':'KL 示例卖场','币种':'MYR','本地价格':800,'价格来源':'原型示例'},{'型号编码':'301','内存':'512GB','卖场':'KL 示例卖场','币种':'MYR','本地价格':3500,'价格来源':'原型示例'}],{name:'演示价格批次',data:null},'贾瑞真','2026-09-16 10:00:00');
 return savePolicy(s,{...blankPolicy(),min:500,max:1500,rate:-5,fixedOn:true,fixed:-20},null,'贾瑞真','2026-09-16 10:00:00');
}
...
function mount(w){
 ...
 const adjustment=p=>[p.rateOn?`比例 ${p.rate>0?'+':''}${p.rate}%`:'',p.fixedOn?`金额 ${p.fixed>0?'+':''}${p.fixed} MYR`:''].filter(Boolean).join('，');
 ...
 function priceHistory(id){const logs=state.batches.flatMap(b=>b.changes.filter(c=>key(c.after)===id).map(c=>({b,c})));modal('本地价格更新记录',table(['批次','原价格（MYR）','更新后（MYR）','卖场 / 来源','操作人 / 时间'],logs.map(({b,c})=>`<tr><td>${b.id}</td><td>${c.before?n(c.before.price):'新增'}</td><td>${n(c.after.price)}</td><td>${esc(c.after.market)}<small>${esc(c.after.source||'—')}</small></td><td>${esc(b.operator)}<small>${esc(b.time)}</small></td></tr>`).join('')));}
 function renderPolicies(){const list=state.policies.filter(p=>!policyFilter||p.status===policyFilter).sort((a,b)=>a.priority-b.priority);$('#priceContent').innerHTML=`<div class="card"><div class="price-toolbar">${field('状态',`<select id="pricePolicyFilter" class="control">${['','启用','停用'].map(x=>opt(x,x||'全部',policyFilter)).join('')}</select>`)}<span class="subtle">数字越小越优先；多条策略不叠加</span><div class="price-spacer"></div>${button('new-policy','＋ 新建策略','',!permission,true)}</div></div><div class="card">${table(['策略编号 / 版本','适用范围','价格区间','调整方式','优先级','状态','更新信息','操作'],list.map(p=>`<tr><td><b>${p.id}</b><small>v${p.version}</small></td><td>${categories.find(c=>c.id===p.category)?.name}<small>${esc(scope(p))}</small><small>${p.memoryMode==='all'?'全部支持内存':esc(p.memories.join('、'))}</small></td><td>${range(p)}</td><td>${adjustment(p)}</td><td>${p.priority}</td><td><span class="tag ${p.status==='启用'?'green':'gray'}">${p.status}</span></td><td>${esc(p.updatedBy)}<small>${esc(p.updatedAt)}</small></td><td><div class="price-actions">${button('edit-policy','编辑',p.id,!permission)}${button('toggle-policy',p.status==='启用'?'停用':'启用',p.id,!permission)}${button('policy-history','历史版本',p.id)}</div></td></tr>`).join(''))}</div>`;}
 function policyEditor(id){if(!permission)return;const old=state.policies.find(p=>p.id===id);editing={id:id||null,draft:copy(old||blankPolicy())};const p=editing.draft;
  modal(id?'编辑价格策略':'新建价格策略',`<p class="price-note">保存为停用版本，检查后在列表启用。每次修改保留历史版本及操作记录。</p><div class="price-grid">${field('策略编号',input('pricePolicyId',id||'保存后自动生成','text','disabled'))}${field('版本',input('pricePolicyVersion',old?'v'+old.version+' → v'+(old.version+1):'v1','text','disabled'))}${field('品类 *',`<select id="priceCategory" class="control">${categories.map(c=>opt(c.id,c.name,p.category)).join('')}</select>`)}${field('型号范围 *',`<select id="priceModelMode" class="control">${opt('all','全部支持型号',p.modelMode)}${opt('selected','指定型号',p.modelMode)}</select>`)}<div id="priceModelChoices" class="price-wide"></div>${field('内存范围 *',`<select id="priceMemoryMode" class="control">${opt('all','全部支持内存',p.memoryMode)}${opt('selected','指定内存',p.memoryMode)}</select>`)}<div id="priceMemoryChoices"></div>${field('物品价下限（MYR，含）*',input('priceMin',p.min,'number','min="0" step="any"'))}${field('物品价上限（MYR，不含）',input('priceMax',p.max??'','number','min="0" step="any" placeholder="留空表示无上限"'))}${field('优先级 *',input('pricePriority',p.priority,'number','min="1" step="1"'))}</div><div class="price-section"><h3>价格调整</h3><p class="subtle">比例和固定金额均以物品价为基础叠加。正数上调，负数下调。</p><div class="price-grid">${field(`<span><input type="checkbox" id="priceRateOn" ${p.rateOn?'checked':''}> 比例调整（%）</span>`,input('priceRate',p.rate,'number','min="-100" step="any"'))}${field(`<span><input type="checkbox" id="priceFixedOn" ${p.fixedOn?'checked':''}> 固定金额加减（MYR）</span>`,input('priceFixed',p.fixed,'number','step="any"'))}</div></div>`,button('save-policy','保存策略','',false,true));renderPolicyScope();syncAdjustments();
 }
 ...
 function syncAdjustments(){$('#priceRate').disabled=!$('#priceRateOn').checked;$('#priceFixed').disabled=!$('#priceFixedOn').checked;}
 function readPolicy(){const p=editing.draft;return {...p,min:$('#priceMin').value===''?NaN:Number($('#priceMin').value),max:$('#priceMax').value===''?null:Number($('#priceMax').value),priority:Number($('#pricePriority').value),rateOn:$('#priceRateOn').checked,rate:$('#priceRate').value===''?NaN:Number($('#priceRate').value),fixedOn:$('#priceFixedOn').checked,fixed:$('#priceFixed').value===''?NaN:Number($('#priceFixed').value)};}
 function policyHistory(id){const p=state.policies.find(x=>x.id===id);modal(id+' · 历史版本',[p,...p.history.slice().reverse()].map((v,i)=>`<section class="price-version"><h3>v${v.version} · ${i?'历史版本':'当前版本'} · ${v.status}</h3><p>${esc(scope(v))} / ${v.memoryMode==='all'?'全部支持内存':esc(v.memories.join('、'))}</p><p>${range(v)}；${adjustment(v)}；优先级 ${v.priority}</p><small>${esc(v.updatedBy)} · ${esc(v.updatedAt)}</small></section>`).join('')+'<h3>操作记录</h3>'+p.logs.slice().reverse().map(l=>`<p>${esc(l.action)} · ${esc(l.operator)} · ${esc(l.time)}</p>`).join(''));}
 ...
 function previewCalculation(){const el=$('#pricePreviewResult');try{const result=calculate(state,{modelId:$('#pricePreviewModel').value,memory:$('#pricePreviewMemory').value,actualP1:Number($('#priceActualP1').value),baselineP1:Number($('#priceBaselineP1').value)});el.innerHTML=`<div class="price-calc-grid">${[['马来本地价格',n(result.localPrice)+' MYR'],['换算系数',n(result.coefficient)],['本机马来价格（物品价）',n(result.itemPrice)+' MYR'],['命中策略',result.strategy?result.strategy.id+' / v'+result.strategy.version:'未命中，使用物品价'],['策略调整',result.strategy?adjustment(result.strategy):'不调整'],['参考报价',n(result.referencePrice)+' MYR']].map(([l,v])=>`<div><span>${l}</span><strong>${esc(v)}</strong></div>`).join('')}</div><p class="subtle">本地价格批次 ${result.batchId}；本次结果仅供预览。</p>`;}catch(e){el.innerHTML=`<p class="price-bad">${esc(e.message)}</p>`;}}
 ...
 async function click(e){const b=e.target.closest('[data-price]');if(!b||b.disabled)return;const a=b.dataset.price,id=b.dataset.id;
  try{if(a==='close'){close();return;}if(a==='tab'){tab=id;render();return;}if(a==='filter'){filter={model:$('#priceModelFilter').value,memory:$('#priceMemoryFilter').value.trim()?normMemory($('#priceMemoryFilter').value):''};renderPrices();return;}if(a==='reset'){filter={model:'',memory:''};renderPrices();return;}
   ...
   if(a==='new-policy')policyEditor();if(a==='edit-policy')policyEditor(id);if(a==='save-policy'&&editing){const next=savePolicy(state,readPolicy(),editing.id,operator,now());if(persist(next)){close();renderPolicies();w.toast('策略已保存，请检查后启用');}}
 ...
 dialog.addEventListener('change',e=>{const t=e.target;if(t.id==='priceFile'){parseUpload(t.files[0]);return;}if(!editing)return;const p=editing.draft;if(t.id==='priceCategory'){p.category=t.value;p.modelIds=[];p.memories=[];renderPolicyScope();}if(t.id==='priceModelMode'){p.modelMode=t.value;p.modelIds=[];p.memories=[];renderPolicyScope();}if(t.id==='priceMemoryMode'){p.memoryMode=t.value;p.memories=[];renderPolicyScope();}if(t.dataset.priceModel){p.modelIds=[...dialog.querySelectorAll('[data-price-model]:checked')].map(x=>x.dataset.priceModel);p.memories=p.memories.filter(m=>memories(p).includes(m));renderPolicyScope();}if(t.dataset.priceMemory)p.memories=[...dialog.querySelectorAll('[data-price-memory]:checked')].map(x=>x.dataset.priceMemory);if(['priceRateOn','priceFixedOn'].includes(t.id))syncAdjustments();});
 w.priceManagementState={getState:()=>copy(state),open:()=>w.setView('priceManagement')};
}
return {KEY,models,headers,normMemory,validateImport,importPrices,blankPolicy,validatePolicy,savePolicy,togglePolicy,calculate,seed,readWorkbook,templateWorkbook,mount};
});
